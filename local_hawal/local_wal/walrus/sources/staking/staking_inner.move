...
public(package) fun can_withdraw_staked_wal_early(
    self: &StakingInnerV1,
    staked_wal: &StakedWal
): bool {
    let wctx = &self.new_walrus_context();
    let node_id = staked_wal.node_id();
    let in_next_committee = self.next_committee.is_some_and!(|cmt| cmt.contains(&node_id));

    staked_wal.can_withdraw_early(in_next_committee, wctx)
}
